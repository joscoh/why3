...
let rec t_equ_simp t1 t2 = match t1.t_node, t2.t_node with
  ...
  | Tapp (s1,l1), Tapp (s2,l2) when BigInt.pos (s1.ls_constr) && BigInt.pos (s2.ls_constr) ->
...
let hs_hash hs = BigInt.to_int (id_hash hs.hs_name) (*JOSH to_int*)
...
let split_record v s cvs =
  let rec fields ll lm cvs vl vfm s = match s.t_node with
    | Tapp (cs,tl) when BigInt.eq cs.ls_constr BigInt.one ->
